...
namespace Labyrinth
{
    public class LabyrinthModel
    {
        ...
        private void Light ()
        {
            ...
            if (isValidPos(playerX - 2, playerY))
            {
                if (table[playerX - 1, playerY] == LabyrinthTable.Field.WALL)
                {
                    if (table[playerX - 2, playerY] == LabyrinthTable.Field.FLOOR) { table.SetField(playerX - 2, playerY, LabyrinthTable.Field.BLACK_FLOOR); }
                    if (table[playerX - 2, playerY] == LabyrinthTable.Field.WALL) { table.SetField(playerX - 2, playerY, LabyrinthTable.Field.BLACK_WALL); }
                    if (table[playerX - 2, playerY] == LabyrinthTable.Field.GOAL) { table.SetField(playerX - 2, playerY, LabyrinthTable.Field.BLACK_GOAL); }
                }
            }
            if (isValidPos(playerX, playerY - 2))
            {
                if (table[playerX, playerY - 1] == LabyrinthTable.Field.WALL)
                {
                    if (table[playerX, playerY - 2] == LabyrinthTable.Field.FLOOR) { table.SetField(playerX, playerY - 2, LabyrinthTable.Field.BLACK_FLOOR); }
                    if (table[playerX, playerY - 2] == LabyrinthTable.Field.WALL) { table.SetField(playerX, playerY - 2, LabyrinthTable.Field.BLACK_WALL); }
                    if (table[playerX, playerY - 2] == LabyrinthTable.Field.GOAL) { table.SetField(playerX, playerY - 2, LabyrinthTable.Field.BLACK_GOAL); }
                }
            }
            if (isValidPos(playerX + 2, playerY))
            {
